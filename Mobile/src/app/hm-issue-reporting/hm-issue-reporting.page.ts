...
@Component({
  selector: 'app-hm-issue-reporting',
  templateUrl: './hm-issue-reporting.page.html',
  styleUrls: ['./hm-issue-reporting.page.scss'],
})
export class HmIssueReportingPage extends BasePage<HMIssueReportingModel> implements OnInit {
  maxDate: any;
  minDate: any;
  submittedRecords: any;
  issueReportingForm: FormGroup;
  issueReportingModel: HMIssueReportingModel;
  ...
  constructor(
    ...
    this.issueReportingModel = new HMIssueReportingModel();
    this.issueReportingForm = this.createHMIssueReportingForm();
  }
  ...
  loadSubmittedRecords() {
    this.loadingController
      ...
      .then((loadEl) => {
        ...
        this.api.selectTableData('GetHMIssueReporting').then((res) => {
    ...
    if (this.issueReportingForm.valid) {
      this.loadingController
        ...
        .then((loadEl) => {
          ...
          this.issueReportingModel = this.hmIssueReportingService.getHMIssueReportingFromFormGroup(this.issueReportingForm);
          ...
          this.issueReportingModel.HMId = this.UserModel.UserTypeId;
          ...
          this.helperService.getCurrentCoordinates().then((res: any) => {
            ...
            if (this.helperService.checkInternetConnection()) {
              this.hmIssueReportingService.createOrUpdateHMIssueReporting(this.issueReportingModel)
                .subscribe((Resp: any) => {
                  loadEl.dismiss();
                  ...
                  this.savingOfflineHMIssueReporting();
                  loadEl.dismiss();
                  console.log('HMIssueReporting errors =>', error);
                });
            } else {
              this.savingOfflineHMIssueReporting();
              loadEl.dismiss();
  ...
  savingOfflineHMIssueReporting() {
    this.helperService.presentToast('Saving Offline...');

    this.api.insertUploadTable('UploadHMIssueReporting', this.issueReportingModel).then(() => {
      this.router.navigateByUrl('/folder/Home');
      this.helperService.presentToast('HM Issue Reporting Conducted Form Saved Offline Successfully.');
    }, (err) => {
      alert(err);
      console.log('HM Issue Reporting Conducted errors =>', err);
    });
  }
  ...
  createHMIssueReportingForm(): FormGroup {
    return this.formBuilder.group({
      HMIssueReportingId: new FormControl(this.issueReportingModel.HMIssueReportingId),
      HMId: new FormControl(this.issueReportingModel.HMId),
